...
Secondary NameNode 는 non-HA enviroment에서 구동된다. HA env 면 사라지는 노드.<br>
...
#### 내부테이블
특징 : 테이블을 삭제하면 테이블 메타정의와 테이블의 데이터까지 같이 삭제되며, ORC 같은 최적화된 형식으로 저장가능하여 성능면에서 좋다.

#### 외부테이블
특징 : 해당 데이터가 하이브 외부에 적재되어 있거나 테이블이 삭제되더라도 원본 데이터가 본래 위치에 있어야 할 때 주로 사용